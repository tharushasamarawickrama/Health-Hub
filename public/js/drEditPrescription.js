document.addEventListener("DOMContentLoaded", () => {
            ...
            <td>
                <div class="sig-codes">
                    <select name="sig_codes[]">
                        <option value=""></option>
                        ${storedSigCodes.map(code => `<option value="${code}">${code}</option>`).join('')}
                    </select>
  ...
  medicationsList.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("add-sig-code")) {
      ...
      let sigOptions = '<option value=""></option>';
      storedSigCodes.forEach((code) => {
        sigOptions += `<option value="${code}">${code}</option>`;
      });
      newSigCode.innerHTML = sigOptions;
      sigCodesDiv.insertBefore(newSigCode, e.target); // Insert before the "+" button
    }
  });

  document
    .getElementById("doctor-prescription-form")
    .addEventListener("submit", function (e) {
      ...
      rows.forEach((row) => {
        ...
        const measurement = row.querySelector(
          '[name="medication_measurement"]'
        ).value;

        const sigCodes = Array.from(
          row.querySelectorAll('[name="sig_codes[]"]')
        )
        ...
        const durationPeriod = row.querySelector(
      ...
      document.getElementById("formatted_prescription_data").value =
